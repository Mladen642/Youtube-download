...
class YouTubeDownloader(QMainWindow):
    def __init__(self):
        ...
        self.ui.open_destination.clicked.connect(self.open_destination)
        self.ui.go_video.clicked.connect(self.video)
        
    def browse(self):
        global x
        x = easygui.diropenbox('Select directory of ytdlp.exe') #x = selected_dir.replace('\\','/')
    ...
    def open_destination(self):
        try:
            subprocess.Popen(f'explorer "{x}"', shell=True)
        except:
